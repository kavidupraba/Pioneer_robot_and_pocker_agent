...
def collectNearestBlock():
    # Find the nearest energy block
    blockHandle, blockName, distance, direction = findEnergyBlocks()[0]

    # Check if the block is within reach (distance threshold) and if there is no obstacle
    if distance <= 0.5:
        # Check for obstacles using the ultrasonic sensors
        obstacle_left = getSensorReading("ultraSonicSensorLeft")
        obstacle_right = getSensorReading("ultraSonicSensorRight")

        # Determine if there's an obstacle in the way
        if obstacle_left > distance and obstacle_right > distance:
            # No obstacles detected in the direct path
            ...
            # Obstacle detected; skip this block
            print("Obstacle detected in path, skipping this block.")
            return 'Obstacle in path, block not collected.'
